...
- 控制台首次登录会弹出「新手引导」浮层（`localStorage` 键 `ai-pay.onboardingTour.v1`，值为 `dismissed` 则不再显示）；侧栏「API 集成」页内嵌**小白向完整流程**（注册/开户 → VA 充值 → 授权白名单 → 调支付 API → 查交易），并附 OpenAPI / `docs/API_INTEGRATION.md` / Node 示例等仓库路径说明，不改变后端行为。
